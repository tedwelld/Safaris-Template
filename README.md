...
Public contact links use `VITE_EMAIL_SUPPORT` and `VITE_WHATSAPP_NUMBER` from `.env.local`. The floating email icon opens the request form; WhatsApp opens the configured business chat. The icons are the installed PrimeIcons WhatsApp brand glyph and envelope. Contact buttons sit at the bottom right, above any visible terms notice. On mobile and tablet, an Explore floating action button at the bottom left opens navigation to all seven pages. It replaces the bottom navigation bar, highlights the current page, and closes after navigation, an outside click, focus leaving the control, or Escape. Desktop header navigation is retained.
