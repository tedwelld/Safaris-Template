    ...
    <section className="section-wrap">
      ...
      <div className="safari-grid">{filtered.map(item => <article className="safari-card" key={item.id}>
        <div className="safari-card-body"><i className={`pi ${item.icon} activity-icon`} aria-hidden="true" /><span className="eyebrow">{item.region} · {item.duration}</span><h2>{item.title}</h2><p>{item.description}</p><p>{item.details}</p><p className="activity-note">{item.note}</p>
          <div className="safari-footer"><span><strong>{item.priceUsd === null ? 'Quote required' : money(item.priceUsd)}</strong><small>{item.priceUsd === null ? 'Ask for a personalised USD quote' : 'per person · USD'}</small></span><Link className="text-link" to={`/booking?trip=${item.id}`}>Select activity <i className="pi pi-arrow-up-right" aria-hidden="true" /></Link></div>
        </div></article>)}</div>
